"""Copy selected local audio into a Reaper import folder."""
...
def export_repo_entry_to_reaper(repo_id: str, drop_dir: Path) -> dict[str, Any]:
    """Copy best local file for one entry into the configured Reaper folder."""
    ...
    out_name = _safe_filename(f"SG2-{title}-{str(row.get('repo_id', 'x'))[:8]}{suffix}", 200)
